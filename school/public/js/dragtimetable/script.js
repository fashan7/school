...
redips.init = function () {
	...
	rd.event.dropped = function () {
        ...
        var dataa = [$('#firstperiodst').val(), $('#periodduration').val(), $('#intervalduration').val(), $('#intervalperiod').val(), $('#noofperiod').val()];
...
//		rd.ajaxCall('TimetableRegister?p=' + rd.obj.id + '_' + pos.join('_')+'&=term' + term +'&year=' + year + '&gradenumber=' + gradenumber + '&gradesection=' + gradesection);
        ...
        $.ajax({
            url: 'TimetableRegister',
            method: 'POST',
            data: {p: pso, term : term, year : year, classno : classno, timetableno: timetableno, dataa:dataa},
