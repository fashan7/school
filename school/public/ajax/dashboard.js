(function ($) {
    ...
    $(document).ready(function () {
            ...
            $.ajax({
            url:"predictionPieChart",
            method:"POST",
            success:function(jsonDatas){
            var plot = $.plot($("#daily-visit-chart"), data, options);
            var mydata = JSON.parse(jsonDatas);
            var countstd = mydata[0].std;
            var countstf = mydata[0].stf;
            var dataPie = [{
                label: "Students",
                data: parseInt(countstd)
            }, {
                label: "Teachers",
                data: parseInt(countstf)
            }];  
            $.plot($(".sm-pie"), dataPie, {
                series: {
                    pie: {
